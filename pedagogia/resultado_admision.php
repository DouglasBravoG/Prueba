...
<html>
    <head>
        ...
        <br>    
        <center><strong><font size = 6>Resultados Generales de Admisi&oacute;n 2017</strong></center>
        <div style="width: 330px; margin: 0 auto;">
            <a data-rel='tooltip' title='Ver Detalle Usuario' class='btn btn-warning' href='resultado_admision_fecha.php'>
        ...
        <hr>
         <center>
            <font size=5 color="gray">
                <?php
                    $query='SELECT  a.noExpediente, a.nota FROM resultado_clave a 
                    INNER JOIN detalle_resultado_clave b ON a.idResultado = b.idResultado 
                    INNER JOIN respuesta_clave c ON b.numeroPregunta = c.numeroPregunta 
                    WHERE a.idClave = 10 AND SUBSTRING(noExpediente,5,1) = 8 GROUP BY noExpediente';
                    $resultado = mysqli_query($conn, $query) or die ('Consulta fallida'.mysql_error());
                    $line = mysqli_fetch_array($resultado, MYSQLI_ASSOC);
                ?>
                Total de Alumnos Evaluados: <strong><font size=5 color="black"> 620       </font></strong>
                Alumnos Con Nota Mayor a 60: <strong><font size=5 color="blue"> 200       </font></strong>
                Alumnos Con Nota Menor a 60: <strong><font size=5 color="red"> 420       </font></strong>
            </font>
        </center>
        <hr>
        ...
        series: [{
            ...
            data: [
                <?php 
                    ...
                    WHERE a.idClave = 10 AND SUBSTRING(noExpediente,5,1) = 8 AND c.idCategoria = 2 GROUP BY resultado ';
                    $resultado = mysqli_query($conn, $query) or die ('Consulta fallida'.mysql_error());
        ...
        series: [{
            ...
            data: [
                <?php 
                    $query = 'SELECT  a.noExpediente,a.idClave, b.numeroPregunta, b.respuesta, COUNT(b.resultado) as total, b.resultado,c.idCategoria 
                    FROM resultado_clave a INNER JOIN detalle_resultado_clave b ON a.idResultado = b.idResultado INNER JOIN respuesta_clave c ON b.numeroPregunta = c.numeroPregunta 
                    WHERE a.idClave = 10 AND SUBSTRING(noExpediente,5,1) = 8 AND c.idCategoria = 3 GROUP BY resultado ';
                    $resultado = mysqli_query($conn, $query) or die ('Consulta fallida'.mysql_error());
                    while ($line = mysqli_fetch_array($resultado, MYSQLI_ASSOC)){
                ?>
                ['<?php if($line ['resultado'] == "Buena"){
        ...
        series: [{
            ...
            data: [
                <?php 
                    $query = 'SELECT  a.noExpediente,a.idClave, b.numeroPregunta, b.respuesta, COUNT(b.resultado) as total, b.resultado,c.idCategoria 
                    FROM resultado_clave a INNER JOIN detalle_resultado_clave b ON a.idResultado = b.idResultado INNER JOIN respuesta_clave c ON b.numeroPregunta = c.numeroPregunta 
                    WHERE a.idClave = 10 AND SUBSTRING(noExpediente,5,1) = 8 AND c.idCategoria = 4 GROUP BY resultado ';
                    $resultado = mysqli_query($conn, $query) or die ('Consulta fallida'.mysql_error());
                    while ($line = mysqli_fetch_array($resultado, MYSQLI_ASSOC)){
                ?>
                ['<?php if($line ['resultado'] == "Buena"){
        ...
        series: [{
            ...
            data: [
                <?php 
                    $query = 'SELECT  a.noExpediente,a.idClave, b.numeroPregunta, b.respuesta, COUNT(b.resultado) as total, b.resultado,c.idCategoria 
                    FROM resultado_clave a INNER JOIN detalle_resultado_clave b ON a.idResultado = b.idResultado INNER JOIN respuesta_clave c ON b.numeroPregunta = c.numeroPregunta 
                    WHERE a.idClave = 10 AND SUBSTRING(noExpediente,5,1) = 8 AND c.idCategoria = 5 GROUP BY resultado';
                    $resultado = mysqli_query($conn, $query) or die ('Consulta fallida'.mysql_error());
                    while ($line = mysqli_fetch_array($resultado, MYSQLI_ASSOC)){
                ?>
                ['<?php if($line ['resultado'] == "Buena"){
            ...
            $('#containerPreguntas').highcharts({
                ...
                series: [{
                    ...
                    data: [<?php
                        ...
                        WHERE a.idClave = 10 AND b.resultado = 'Buena' AND SUBSTRING(noExpediente,5,1) = 8 GROUP BY numeroPregunta";
                        $resultado = mysqli_query($conn, $query) or die ('Consulta fallida'.mysql_error());
                        while ($line = mysqli_fetch_array($resultado, MYSQLI_ASSOC)){
                        ?>
                        [<?php  $porcen = ($line ['totBuenas'] * 100 ) / 620;
                             echo round($porcen,2);?>],
                        <?php
                        }
                        ?>]
                    ...
                    data: [<?php
                        ...
                        FROM resultado_clave a INNER JOIN detalle_resultado_clave b ON a.idResultado = b.idResultado
                        WHERE a.idClave = 10 AND b.resultado = 'Mala' AND SUBSTRING(noExpediente,5,1) = 8 GROUP BY numeroPregunta";
                        $resultado = mysqli_query($conn, $query) or die ('Consulta fallida'.mysql_error());
                        while ($line = mysqli_fetch_array($resultado, MYSQLI_ASSOC)){
                        ?>
                        [<?php $porcen = ($line ['totMalas'] * 100 ) / 620;
                             echo round($porcen,2);?>],
                        <?php
                        }
                        ?>]
